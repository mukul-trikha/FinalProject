...
if(isset($_POST['category'])){
	...
	//$querySearch=mysqli_query("SELECT * FROM Doctors where specialization LIKE '%$searchq%' OR DoctorName LIKE '%$searchq%'" ) or die ("could not search");
	$count =mysqli_num_rows($querySearch);
	if($count == 0){
		$output='There are no doctors available in this category';
	}
	else{
		while($row=mysqli_fetch_array($querySearch)){
			$specialization =$row['Specialization'];
			$output.='<div>'.$specialization.'</div>';
									...
									<div class="tg-directpost">
										...
										<div class="tg-directinfo">
											<div class="tg-directposthead">
												<h3><a href="doctor-detail.php">Dr. Steve Northrop</a></h3>
